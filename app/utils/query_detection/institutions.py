...
try:
    coordinates_df = pd.read_excel(PATHS["hospital_coordinates_path"])
except Exception as e:
    logger.error(f"Failed to load hospital coordinates Excel: {e}")
    raise

def get_institution_list(coordinates_df):
    """
    ...
    column_1 = coordinates_df.iloc[:, 0]
...
institution_list = get_institution_list(coordinates_df)